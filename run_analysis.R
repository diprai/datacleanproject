...
#Tidy data as output as data_new file

data.new <- aggregate(data.sub[,3:81], by = list(activity = data.sub$activity, subject = data.sub$subject),FUN = mean)
write.table(x = data.new, file = "data_new.txt", row.names = FALSE)
str(data.new)
